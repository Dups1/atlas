...
class servicioPerfilFirebase {
  ...
  servicioPerfilFirebase({autenticacionStorage? storage, String? baseUrl})
  ...
  Future<Map<String, dynamic>> _fetchPerfilDesdeColeccion(
    ...
    if (response.statusCode != 200) {
      throw Exception(
        'Error al cargar colección (${response.statusCode}): ${response.body}',
      );
    }
    ...
    if (data is Map<String, dynamic>) {
      ...
      throw Exception('Perfil de colección no coincide con la sesión');
    }
    throw Exception('Respuesta inválida de colección de usuarios');
  }

  Future<Map<String, dynamic>> fetchPerfil({String? idToken}) async {
    final token = idToken ?? await _storage.recuperarToken();
    ...
    return _fetchPerfilDesdeColeccion(token, uidSesion, emailSesion);
  }
  ...
  Future<void> actualizarPerfil(
    ...
    final response = await http.patch(
      Uri.parse('$baseUrl/firebase/usuarios/$userId'),
      headers: headers,
